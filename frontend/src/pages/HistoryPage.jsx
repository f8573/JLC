...
function MatrixPreviewSmall({ matrixString }) {
  ...
  if (rows === 0) return <div className="px-3 py-2 bg-slate-50 rounded-sm">Empty</div>
          ...
          <div className="p-8 max-w-7xl mx-auto w-full">
            ...
            <div className="space-y-3">
              ...
              {visibleHistory.map((h, idx) => (
                <div key={h.ts || idx} onClick={() => { import('../utils/navigation').then(m => m.navigate(`/matrix=${encodeURIComponent(h.title)}/basic`)) }} className="bg-white border border-border-color rounded-xl p-4 flex items-center gap-6 hover:border-primary/30 transition-all group shadow-sm cursor-pointer">
                  <div className="w-32 flex justify-center shrink-0">
                    <MatrixPreviewSmall matrixString={h.title} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-bold text-slate-900 truncate">{(h.title || '').slice(0, 60)}</h3>
                    <p className="text-xs text-slate-500 mt-0.5">Operation: <span className="text-primary font-medium">Viewed</span></p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-[11px] font-medium text-slate-400 block mb-1">{h.ts ? new Date(h.ts).toLocaleString() : ''}</span>
                    <div className="flex items-center gap-2 justify-end">
                      <span className="text-[10px] font-mono font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">{h.rows || ''}x{h.cols || ''}</span>
                    </div>
                  </div>
                  <div className="pl-4 border-l border-slate-100 shrink-0">
                    <button onClick={(e) => { e.stopPropagation(); import('../utils/navigation').then(m => m.navigate(`/matrix=${encodeURIComponent(h.title)}/basic`)) }} className="flex items-center gap-1.5 px-4 py-1.5 text-primary bg-purple-light hover:bg-primary hover:text-white text-xs font-bold rounded-lg transition-all">
            ...
            {hasMore && (
              <div className="mt-8 flex justify-center">
                <button 
                  ...
                  className="text-sm font-semibold text-slate-500 hover:text-primary transition-colors flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-primary/5"
                >
