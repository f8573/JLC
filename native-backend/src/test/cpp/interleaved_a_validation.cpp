// K-major A layout regression cases; retains the existing independent long-double oracle.
...
static bool one_case(const Case& x, bool pool, std::uint64_t seed, Totals& total,
    ...
    for (int pass = 0; pass < 4; ++pass) {
        const char* mode = pass < 2 ? "row" : "kmajor6";
        setenv("JLC_NATIVE_A_LAYOUT", mode, 1);
        setenv("JLC_NATIVE_AVX2_MICROKERNEL", pass == 0 ? "6x8-noinline" : "6x8-u4", 1);
        c.data = initial;
        if (profile_counts) {
...
int main() {
    ...
    for (int k : {1,2,3,4,5,7,8,9,17,255,256,257,513}) {
        ...
        for(const char* scheduler : {"panel","shared-tail"}) {
            ...
            for(const char* packing : {"private","l3"}) {
                ...
                for(int repeat=0;repeat<3;++repeat)
                    if(!one_case(cases.back(),pool,7001+repeat,total,true))return 22;
            }
        }
