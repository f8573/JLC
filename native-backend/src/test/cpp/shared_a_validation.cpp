// Packed-A ownership regression cases; extends the existing shared-tail audit oracle.
#include "jlc_native.h"

#include <algorithm>
#include <atomic>
#include <cmath>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <future>
#include <random>
#include <string>
#include <thread>
#include <vector>

struct Case {
    int m, k, n, threads;
    bool ta = false, tb = false, ca = false, cb = false, cc = false;
    double alpha = -0.625, beta = 0.375;
    const char* label = "";
};

struct Matrix {
    int logical_rows, logical_cols, rows, cols, ld, offset;
    bool transpose, col_major;
    std::vector<double> data;

    Matrix(int r, int c, bool t, bool col, int extra = 5)
        : logical_rows(r), logical_cols(c), rows(t ? c : r), cols(t ? r : c),
          ld((col ? rows : cols) + extra), offset(13), transpose(t), col_major(col),
          data(offset + static_cast<std::size_t>(ld) * (col ? cols : rows) + 23,
               -991.125) {}

    std::size_t index(int i, int j) const {
        if (transpose) std::swap(i, j);
        return offset + (col_major ? static_cast<std::size_t>(j) * ld + i
                                   : static_cast<std::size_t>(i) * ld + j);
    }
    double get(int i, int j) const { return data[index(i, j)]; }
};

struct Totals {
    std::uint64_t cases = 0, bitwise = 0, oracle = 0, calls = 0;
    double worst_abs = 0.0, worst_rel = 0.0;
};

static int a_flags(const Case& x) {
    return (x.ta ? JLC_GEMM_FLAG_A_TRANSPOSE : 0) |
           (x.ca ? JLC_GEMM_FLAG_A_COL_MAJOR : 0);
}
static int b_flags(const Case& x) {
    return (x.tb ? JLC_GEMM_FLAG_B_TRANSPOSE : 0) |
           (x.cb ? JLC_GEMM_FLAG_B_COL_MAJOR : 0);
}
static int c_flags(const Case& x) { return x.cc ? JLC_GEMM_FLAG_C_COL_MAJOR : 0; }

static jlc_status call(const Case& x, Matrix& a, Matrix& b, Matrix& c) {
    return jlc_native_gemm_strided(
        a.data.data(), a.offset, a.ld, a.rows, a.cols, a_flags(x),
        b.data.data(), b.offset, b.ld, b.rows, b.cols, b_flags(x),
        c.data.data(), c.offset, c.ld, c.rows, c.cols, c_flags(x),
        x.alpha, x.beta, x.threads, JLC_GEMM_FLAG_FORCE_BUILTIN);
}

static bool one_case(const Case& x, bool pool, std::uint64_t seed, Totals& total,
                     bool profile_counts = false) {
    std::mt19937_64 rng(seed);
    std::uniform_real_distribution<double> dist(-0.75, 0.75);
    Matrix a(x.m, x.k, x.ta, x.ca), b(x.k, x.n, x.tb, x.cb),
           c(x.m, x.n, false, x.cc, 9);
    for (int i = 0; i < x.m; ++i)
        for (int p = 0; p < x.k; ++p) a.data[a.index(i, p)] = dist(rng);
    for (int p = 0; p < x.k; ++p)
        for (int j = 0; j < x.n; ++j) b.data[b.index(p, j)] = dist(rng);
    for (int i = 0; i < x.m; ++i)
        for (int j = 0; j < x.n; ++j) c.data[c.index(i, j)] = dist(rng);
    const auto initial = c.data;
    std::vector<double> panel;
    jlc_gemm_profile p_panel{}, p_shared{};

    for (int pass = 0; pass < 2; ++pass) {
        const char* mode = pass == 0 ? "private" : "l3";
        setenv("JLC_NATIVE_A_PACKING", mode, 1);
        c.data = initial;
        if (profile_counts) {
            jlc_native_profile_reset();
            jlc_native_profile_set_enabled(true);
        }
        const auto status = call(x, a, b, c);
        ++total.calls;
        if (profile_counts) {
            jlc_native_profile_snapshot(pass == 0 ? &p_panel : &p_shared);
            jlc_native_profile_set_enabled(false);
        }
        if (status != JLC_STATUS_SUCCESS) {
            std::fprintf(stderr, "FAIL status label=%s pool=%d mode=%s status=%d\n",
                         x.label, pool, mode, static_cast<int>(status));
            return false;
        }
        if (pass == 0) panel = c.data;
        else if (std::memcmp(panel.data(), c.data.data(), c.data.size() * sizeof(double)) != 0) {
            std::fprintf(stderr, "FAIL bitwise label=%s pool=%d\n", x.label, pool);
            return false;
        }
    }

    total.bitwise += static_cast<std::uint64_t>(x.m) * x.n;
    const std::uint64_t elements = static_cast<std::uint64_t>(x.m) * x.n;
    const std::uint64_t target = elements < 25000 ? elements : 257;
    for (std::uint64_t s = 0; s < target; ++s) {
        const std::uint64_t q = target == elements ? s : (s * (elements - 1)) / (target - 1);
        const int i = static_cast<int>(q / x.n), j = static_cast<int>(q % x.n);
        long double sum = 0.0L;
        for (int p = 0; p < x.k; ++p)
            sum += static_cast<long double>(a.get(i, p)) * static_cast<long double>(b.get(p, j));
        const long double expected_ld = static_cast<long double>(x.alpha) * sum +
                                        static_cast<long double>(x.beta) * initial[c.index(i, j)];
        const double expected = static_cast<double>(expected_ld);
        const double got = c.get(i, j);
        const double abs_err = std::abs(got - expected);
        const double rel_err = abs_err / std::max(1.0, std::abs(expected));
        total.worst_abs = std::max(total.worst_abs, abs_err);
        total.worst_rel = std::max(total.worst_rel, rel_err);
        ++total.oracle;
        if (!std::isfinite(got) || rel_err > 3e-12) {
            std::fprintf(stderr, "FAIL oracle label=%s pool=%d i=%d j=%d got=%.17g expected=%.17g rel=%.3g\n",
                         x.label, pool, i, j, got, expected, rel_err);
            return false;
        }
    }

    auto guards = c.data;
    for (int i = 0; i < x.m; ++i)
        for (int j = 0; j < x.n; ++j) guards[c.index(i, j)] = initial[c.index(i, j)];
    if (std::memcmp(guards.data(), initial.data(), initial.size() * sizeof(double)) != 0) {
        std::fprintf(stderr, "FAIL guard label=%s pool=%d\n", x.label, pool);
        return false;
    }

    if (profile_counts) {
        const bool equal = p_panel.pack_b_calls == p_shared.pack_b_calls &&
            p_panel.pack_b_bytes == p_shared.pack_b_bytes &&
            p_panel.microtile_calls == p_shared.microtile_calls;
        for (int q = 0; q < 2; ++q) {
            const auto& p = q ? p_shared : p_panel;
            std::printf("COUNTS label=%s pool=%d mode=%s A=%llu A_bytes=%llu B=%llu B_bytes=%llu micro=%llu actual=%llu panels=%llu mc=%llu kc=%llu nc=%llu mr=%llu nr=%llu vendor=%llu\n",
                x.label, pool, q ? "l3" : "private",
                (unsigned long long)p.pack_a_calls, (unsigned long long)p.pack_a_bytes,
                (unsigned long long)p.pack_b_calls, (unsigned long long)p.pack_b_bytes,
                (unsigned long long)p.microtile_calls, (unsigned long long)p.last_actual_threads,
                (unsigned long long)p.last_panel_count, (unsigned long long)p.last_mc,
                (unsigned long long)p.last_kc, (unsigned long long)p.last_nc,
                (unsigned long long)p.last_mr, (unsigned long long)p.last_nr,
                (unsigned long long)p.vendor_calls);
        }
        if (!equal) return false;
        if (x.m == 2048 && x.k == 2048 && x.n == 2048) {
            const int copies = x.threads == 1 ? 16 : x.threads == 12 ? 7 : 4;
            if (p_panel.pack_a_calls != 43776 || p_panel.pack_a_bytes != 536870912 ||
                p_shared.pack_a_calls != 2736ULL * copies ||
                p_shared.pack_a_bytes != 33554432ULL * copies ||
                p_shared.pack_b_calls != 128 || p_shared.pack_b_bytes != 33554432 ||
                p_shared.microtile_calls != 700416 || p_shared.last_actual_threads != (unsigned)x.threads)
                return false;
        }
    }
    ++total.cases;
    std::printf("PASS label=%s pool=%d m=%d k=%d n=%d threads=%d\n",
                x.label, pool, x.m, x.k, x.n, x.threads);
    return true;
}

static int adversarial() {
    std::vector<Case> cases;
    for (int w : {1,8,12,16}) {
        cases.push_back({67,513,2048,w,false,false,false,false,false,-0.625,0.375,"multiple-kc-m-tail"});
        cases.push_back({71,259,2177,w,false,false,false,false,false,1.0,-0.25,"incomplete-wave-n-tail"});
        cases.push_back({2053,257,2049,w,false,false,false,false,false,0.875,-0.25,"multiple-mc-kc"});
        cases.push_back({47,515,2049,w,true,true,false,false,false,-0.9,-0.2,"transpose-both"});
        cases.push_back({59,263,2049,w,true,false,true,true,false,0.55,0.4,"column-major-ab"});
    }
    cases.push_back({3,8193,2049,12,false,false,false,false,false,-0.7,0.2,"empty-pack-ranks"});
    cases.push_back({73,257,385,7,false,false,false,false,true,0.8,0.3,"column-major-c"});
    // Irregular domain membership and singleton domains on the host's CPU map.
    for (int w : {2,3,5,7,10,11,13,15})
        cases.push_back({43,513,2049,w,false,true,false,false,false,-0.75,0.25,"irregular-domain"});
    Totals total;
    for (bool pool : {false, true}) {
        jlc_context_handle ctx = pool ? jlc_native_context_create(16,64,0) : 0;
        if (pool && !ctx) return 20;
        for (const char* scheduler : {"panel", "shared-tail"}) {
            setenv("JLC_NATIVE_GEMM_SCHEDULER", scheduler, 1);
            std::printf("SCHEDULER %s pool=%d\n", scheduler, pool);
            for (std::size_t i=0;i<cases.size();++i)
                if (!one_case(cases[i],pool,0x5a17ULL+i*7919+pool,total,true)) return 21;
        }
        if (ctx) jlc_native_context_destroy(ctx);
    }
    std::printf("ADVERSARIAL_SUMMARY cases=%llu calls=%llu bitwise_outputs=%llu oracle_outputs=%llu worst_abs=%.17g worst_rel=%.17g\n",
        (unsigned long long)total.cases,(unsigned long long)total.calls,
        (unsigned long long)total.bitwise,(unsigned long long)total.oracle,total.worst_abs,total.worst_rel);
    return 0;
}

static int accounting() {
    Totals total;
    for (bool pool : {false,true}) {
        auto ctx=pool?jlc_native_context_create(16,64,0):0;
        for (int w : {1,8,12,16}) {
            setenv("JLC_NATIVE_GEMM_SCHEDULER", w==12?"shared-tail":"panel",1);
            Case x{2048,2048,2048,w,false,false,false,false,false,1.0,0.0,"accounting-2048"};
            if(!one_case(x,pool,98765+w,total,true)) return 22;
        }
        if(ctx)jlc_native_context_destroy(ctx);
    }
    std::printf("ACCOUNTING_PASS cases=%llu bitwise_outputs=%llu oracle_outputs=%llu\n",
        (unsigned long long)total.cases,(unsigned long long)total.bitwise,(unsigned long long)total.oracle);
    return 0;
}

static int stress() {
    Totals total;
    const std::vector<int> workers{1,8,12,16,5,13};
    jlc_context_handle ctx = jlc_native_context_create(16,64,0);
    if (!ctx) return 30;
    for (int q = 0; q < 300; ++q) {
        Case x{59 + q%17, 249 + 2*(q%13), 1921 + 128*(q%3), workers[q%workers.size()]};
        x.alpha = (q%2 ? -0.65 : 0.85); x.beta = (q%3 ? 0.2 : -0.3); x.label="stress";
        setenv("JLC_NATIVE_GEMM_SCHEDULER",q%2?"shared-tail":"panel",1);
        // Alternate selectors on the same workspace, varying sizes and worker count.
        if (!one_case(x, true, 0x98127ULL + q*17, total, false)) return 31;
    }
    jlc_native_context_destroy(ctx);
    std::printf("STRESS_SUMMARY logical_cases=%llu gemm_calls=%llu bitwise_outputs=%llu oracle_outputs=%llu worst_rel=%.17g timeouts=0 deadlocks=0 crashes=0 exceptions=0 incorrect=0\n",
        (unsigned long long)total.cases, (unsigned long long)total.calls,
        (unsigned long long)total.bitwise, (unsigned long long)total.oracle, total.worst_rel);
    return 0;
}

static bool concurrent_pair(bool pool) {
    jlc_context_handle ctx = pool ? jlc_native_context_create(12,64,0) : 0;
    setenv("JLC_NATIVE_GEMM_SCHEDULER","shared-tail",1);
    setenv("JLC_NATIVE_A_PACKING","l3",1);
    auto task = [](int seed) {
        Case x{97 + seed, 263, 1665, 12,false,false,false,false,false,-0.4,0.3,"concurrent"};
        std::mt19937_64 rng(7001 + seed);
        std::uniform_real_distribution<double> d(-.5,.5);
        Matrix a(x.m,x.k,false,false), b(x.k,x.n,false,false), c(x.m,x.n,false,false,7);
        for(int i=0;i<x.m;++i)for(int p=0;p<x.k;++p)a.data[a.index(i,p)]=d(rng);
        for(int p=0;p<x.k;++p)for(int j=0;j<x.n;++j)b.data[b.index(p,j)]=d(rng);
        for(int i=0;i<x.m;++i)for(int j=0;j<x.n;++j)c.data[c.index(i,j)]=d(rng);
        auto initial=c.data;
        if(call(x,a,b,c)!=JLC_STATUS_SUCCESS)return false;
        for(int s=0;s<41;++s){int i=(s*(x.m-1))/40,j=((40-s)*(x.n-1))/40;long double sum=0;
            for(int p=0;p<x.k;++p)sum+=(long double)a.get(i,p)*b.get(p,j);
            double want=(double)(x.alpha*sum+x.beta*initial[c.index(i,j)]);
            if(std::abs(c.get(i,j)-want)/std::max(1.,std::abs(want))>3e-12)return false;}
        return true;
    };
    auto f1=std::async(std::launch::async,task,1);
    auto f2=std::async(std::launch::async,task,2);
    const bool ok1=f1.get(), ok2=f2.get();
    const bool ok=ok1&&ok2;
    if(ctx)jlc_native_context_destroy(ctx);
    std::printf("CONCURRENT pool=%d calls=2 result=%s\n",pool,ok?"PASS":"FAIL");
    return ok;
}

static int topology_control(int workers) {
    Totals total;
    for (bool pool : {false,true}) {
        const auto ctx=pool?jlc_native_context_create(workers,64,0):0;
        setenv("JLC_NATIVE_GEMM_SCHEDULER","shared-tail",1);
        for (int q=0;q<3;++q) {
            Case x{67+q,513,workers*128+1,workers,false,false,false,false,false,-.7,.2,"topology-control"};
            if (!one_case(x,pool,12121+q,total,true)) return 51;
        }
        if(ctx)jlc_native_context_destroy(ctx);
    }
    std::printf("TOPOLOGY_CONTROL_PASS cases=%llu bitwise_outputs=%llu\n",
        (unsigned long long)total.cases,(unsigned long long)total.bitwise);
    return 0;
}

int main(int argc,char**argv) {
    const std::string mode=argc>1?argv[1]:"adversarial";
    if(mode=="topology")return topology_control(argc>2?std::atoi(argv[2]):4);
    if(mode=="adversarial")return adversarial();
    if(mode=="accounting")return accounting();
    if(mode=="stress")return stress();
    if(mode=="concurrent")return concurrent_pair(false)&&concurrent_pair(true)?0:40;
    return 2;
}
